...
class Rm(name: String) extends Command {
  ...
  override def apply(state: State): State = {
    ...
    else if (wd.path.equals(absolutePath))
      state.setMessage("You can't delete the folder in which you are")
    else if (wd.path.contains(absolutePath))
      state.setMessage("You can't delete the folder above current")
    else
